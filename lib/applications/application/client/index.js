module.exports = require('async')(function *(resolve, reject, application, languages, specs, path, runtime, events) {
    "use strict";
    ...
    let appBuildPath;
    appBuildPath = (typeof specs.path === 'string') ? specs.path : 'applications/client/' + application.name;
    appBuildPath = require('path').join(path, appBuildPath);

    if (!specs.path && specs.mode === 'phonegap') {
        appBuildPath = require('path').join(appBuildPath, 'phonegap');
    }
    else if (!specs.path && specs.mode !== 'phonegap') {
    ...
    if (specs.mode === 'phonegap') {
        ...
        yield require('./languages.js')(application, languages, specs, appBuildPath, events);
